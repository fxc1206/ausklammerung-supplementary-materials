...
**信息密度因此并非物理长度的代理变量，而是独立的预测维度**（详见下方附图 A1）。

![附图 A1 · 10 特征模型 SHAP 贡献排名：累积惊奇度与成分长度的对比](../figures/figA1_density_vs_weight.png)

> *附图 A1 · 10 特征模型 SHAP 贡献排名：累积惊奇度与成分长度的对比（红色标注对比特征）*
...
上述证据表明信息密度是超越长度的预测维度，但需排除一个替代假说——**剩余变异或许来自结构复杂度而非信息密度**。
...
| 证据 | 来源 | 结论 |
|:---|:---|:---|
| 控制词数后，含额外关系从句的介词短语与不含的版本在外置可接受度上无差异 | Weber, 2019 | 复杂度独立于长度无效应 |
| 仅含 3–6 词的短介词短语在分词结构中规律性出现在后场，动因为语义邻接而非形式重量 | Vinckel, 2006: 82 | 复杂度不能解释短成分外置 |

**两项证据共同表明**：物理长度与结构复杂度均不能解释外置选择的全部变异，而本文数据中**惊奇度在控制长度后仍保留近乎完整的预测力**（见本节首段数值）。惊奇度作为信息论意义上的可预测性度量，其所捕捉的剩余变异因此归属于**信息密度维度**。
